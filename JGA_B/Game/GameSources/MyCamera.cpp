...
namespace basecross {
	...
	void MyCamera::SetCameraArm(float f) {
		//前回のターンからの時間
		float elapsedTime = App::GetApp()->GetElapsedTime();
		...
		if (f > m_ArmLen) {
			m_ArmLen += elapsedTime * m_ZoomSpeed;
		}
		...
		if (ptrTarget) {
			...
			Vec3 toAt = ptrTarget->GetComponent<Transform>()->GetWorldMatrix().transInMatrix();
			toAt += m_TargetToAt;
			newAt = Lerp::CalculateLerp(GetAt(), toAt, 0, 1.0f, 1.0, Lerp::EaseOut);
		}
		////目指したい場所にアームの値と腕ベクトルでEyeを調整
		Vec3 toEye = newAt + armVec * m_ArmLen;
		newEye = Lerp::CalculateLerp(GetEye(), toEye, 0, 1.0f, m_ToTargetLerp, Lerp::EaseOut);
	}
	//--------------------------------------------------------------------------------------
	...
	void OpeningCamera::OnUpdate() {
		auto ptrOpeningCameraman = dynamic_pointer_cast<CameraTargetObject>(GetCameraObject());
	...
	void OpeningCamera::SetEye(const bsm::Vec3& Eye) {
		Camera::SetEye(Eye);
	}
	void OpeningCamera::SetEye(float x, float y, float z) {
		Camera::SetEye(x, y, z);
	}
}
//end basecross
